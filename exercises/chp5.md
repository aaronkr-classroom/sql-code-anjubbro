...
6. 집계 함수는 그룹 함수와 밀접한 관련성이 있으며 SELECT절이나 ( **HAVIG** )절에만 사용할 수 있다.
7. SELECT절에서 연산이냐 집계 함수의 적용 결과로 새로운 값을 반환할 때 새로운 이름인 별칭을 정의하는 키워드는 ( **AS** )이다.
8. ( **부 질의문** )은 질의문 안에 중첩 (nested) 되어 포함된 또 다른 SELECT 검색문을 말한다.
...
11. 중첩 질의문 중에서 내부 질의의 WHERE절 검색조건식이 외부 질의에 선언된 테이블의 일부 열을 참조하는 질의를 ( **상관 부 질의문** )이라고 한다.
