...
**장점 : 검색 성능이 크게 향상된다 / 특정 조건을 만족하는 데이터를 빠르게 찾을 수 있다 / 데이터베이스에서 정렬이나 그룹핑 연산 시에도 성능이 향상된다.**
**단점 : 인덱스를 너무 많이 생성하면 삽입, 삭제, 갱신 시 성능이 저하될 수 있다 / 인덱스 자체도 공간을 차지하며 유지 관리가 필요하다.**

**18 인덱스를 사용하는 것이 바람직한 경우와 그렇지 않은 경우를 구분하여 설명하시오.**

**바람직한 경우 : 자주 조회되거나 검색 조건에 자주 사용되는 열에 인덱스를 사용하는 것이 좋다 / 대량의 데이터를 대상으로 검색 속도를 높이고자 할 때 유용하다.**
**바람직하지 않은 경우 : 자주 변경되는 열에 인덱스를 사용하면 수정, 삭제 성능이 저하될 수 있다 / 데이터 양이 적거나 테이블이 자주 수정되는 경우에는 오히려 성능이 떨어질 수 있다.**
